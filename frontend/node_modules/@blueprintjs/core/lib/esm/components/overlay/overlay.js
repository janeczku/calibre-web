...
export class Overlay extends AbstractPureComponent {
    ...
    startFocusTrapElement = createRef();
    // An empty, keyboard-focusable div at the end of the Overlay content
    endFocusTrapElement = createRef();
    render() {
        ...
        _jsx("div", { "aria-live": "polite", className: containerClasses, onKeyDown: this.handleKeyDown, ref: this.containerElement, children: _jsx(TransitionGroup, { appear: true, component: null, children: childrenWithTransitions }) }));
        if (usePortal) {
            return (_jsx(Portal, { className: this.props.portalClassName, container: this.props.portalContainer, stopPropagationEvents: this.props.portalStopPropagationEvents, children: transitionGroup }));
        }
        else {
    ...
    maybeRenderChild = (child) => {
        ...
        const decoratedChild = typeof child === "object" ? (cloneElement(child, {
            ...
            tabIndex,
        })) : (_jsx("span", { className: Classes.OVERLAY_CONTENT, tabIndex: tabIndex, children: child }));
        const { onOpening, onOpened, onClosing, transitionDuration, transitionName } = this.props;
        return (_jsx(CSSTransition, { classNames: transitionName, onEntering: onOpening, onEntered: onOpened, onExiting: onClosing, onExited: this.handleTransitionExited, timeout: transitionDuration, addEndListener: this.handleTransitionAddEnd, children: decoratedChild }));
    };
    maybeRenderBackdrop() {
        const { backdropClassName, backdropProps, hasBackdrop, isOpen, transitionDuration, transitionName } = this.props;
        if (hasBackdrop && isOpen) {
            return (_jsx(CSSTransition, { classNames: transitionName, timeout: transitionDuration, addEndListener: this.handleTransitionAddEnd, children: _jsx("div", { ...backdropProps, className: classNames(Classes.OVERLAY_BACKDROP, backdropClassName, backdropProps?.className), onMouseDown: this.handleBackdropMouseDown }) }, "__backdrop"));
        }
        else {
            return null;
        }
    ...
    renderDummyElement(key, props) {
        ...
        return (_jsx(CSSTransition, { classNames: transitionName, addEndListener: this.handleTransitionAddEnd, timeout: transitionDuration, unmountOnExit: true, children: _jsx("div", { tabIndex: 0, ...props }) }, key));
    }
    /**
