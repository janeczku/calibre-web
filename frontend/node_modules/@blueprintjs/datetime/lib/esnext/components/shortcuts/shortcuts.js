...
export const DatePickerShortcutMenu = props => {
    ...
    const shortcuts = props.shortcuts === true
        ...
        : props.shortcuts;
    return (_jsx(Menu, { "aria-label": "Date picker shortcuts", className: Classes.DATERANGEPICKER_SHORTCUTS, tabIndex: 0, children: shortcuts.map((shortcut, index) => (_jsx(ShortcutMenuItem, { active: selectedShortcutIndex === index, index: index, maxDate: maxDate, minDate: minDate, onShortcutClick: onShortcutClick, shortcut: shortcut }, index))) }));
};
const ShortcutMenuItem = props => {
    ...
    return (_jsx(MenuItem, { active: active, disabled: !isShortcutInRange, onClick: handleClick, shouldDismissPopover: false, text: shortcut.label }));
};
