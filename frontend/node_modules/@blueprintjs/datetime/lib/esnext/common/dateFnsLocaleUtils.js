 ...
 */
export async function loadDateFnsLocale(localeCode) {
    try {
 ...
 */
export function useDateFnsLocale(localeOrCode, dateFnsLocaleLoader = loadDateFnsLocale) {
    // make sure to set the locale correctly on first render if it is available
    const [locale, setLocale] = useState(typeof localeOrCode === "object" ? localeOrCode : undefined);
    useEffect(() => {
