...
export function useMonthSelectRightOffset(monthSelectElement, containerElement, displayedMonthText) {
    const [monthRightOffset, setMonthRightOffset] = useState(0);
    useIsomorphicLayoutEffect(() => {
        if (containerElement.current == null) {
            return;
        }
        // measure width of text as rendered inside our container element.
        const monthTextWidth = DatePickerUtils.measureTextWidth(displayedMonthText, Classes.DATEPICKER_CAPTION_MEASURE, containerElement.current);
