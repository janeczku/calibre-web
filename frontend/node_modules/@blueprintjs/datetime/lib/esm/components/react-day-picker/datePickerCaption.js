 ...
 */
export const DatePickerCaption = (props) => {
    ...
    const monthSelectElement = useRef(null);
    const { currentMonth, goToMonth, nextMonth, previousMonth } = useNavigation();
    const handlePreviousClick = useCallback(() => previousMonth && goToMonth(previousMonth), [previousMonth, goToMonth]);
    const handleNextClick = useCallback(() => nextMonth && goToMonth(nextMonth), [nextMonth, goToMonth]);
    const prevButton = (_jsx(Button, { "aria-label": labels.labelPrevious(previousMonth, { locale }), className: classNames(CaptionClasses.DATEPICKER3_NAV_BUTTON, CaptionClasses.DATEPICKER3_NAV_BUTTON_PREVIOUS), disabled: !previousMonth, icon: _jsx(ChevronLeft, {}), onClick: handlePreviousClick, variant: "minimal" }));
    const nextButton = (_jsx(Button, { "aria-label": labels.labelNext(nextMonth, { locale }), className: classNames(CaptionClasses.DATEPICKER3_NAV_BUTTON, CaptionClasses.DATEPICKER3_NAV_BUTTON_NEXT), disabled: !nextMonth, icon: _jsx(ChevronRight, {}), onClick: handleNextClick, variant: "minimal" }));
    ...
    const monthSelect = (_jsx(HTMLSelect, { "aria-label": labels.labelMonthDropdown(), iconProps: { style: { right: monthSelectRightOffset } }, className: classNames(CaptionClasses.DATEPICKER3_DROPDOWN_MONTH, rdpClassNames.dropdown_month), minimal: true, onChange: handleMonthSelectChange, ref: monthSelectElement, value: displayMonth, options: availableMonthOptions }, "month"));
    ...
    const yearSelect = (_jsx(HTMLSelect, { "aria-label": labels.labelYearDropdown(), className: classNames(CaptionClasses.DATEPICKER3_DROPDOWN_YEAR, rdpClassNames.dropdown_year), minimal: true, onChange: handleYearSelectChange, value: displayYear, options: allYearOptions }, "year"));
    const orderedSelects = reverseMonthAndYearMenus ? [yearSelect, monthSelect] : [monthSelect, yearSelect];
    const hiddenCaptionLabel = (_jsx("div", { className: ReactDayPickerClasses.RDP_VHIDDEN, children: _jsx(CaptionLabel, { displayMonth: props.displayMonth, id: props.id }) }));
    return (_jsxs("div", { className: classNames(CaptionClasses.DATEPICKER3_CAPTION, rdpClassNames.caption), ref: containerElement, children: [hiddenCaptionLabel, prevButton, orderedSelects, nextButton] }));
};
