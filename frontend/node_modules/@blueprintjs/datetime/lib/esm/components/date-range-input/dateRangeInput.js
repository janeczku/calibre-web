...
import { Boundary, Classes as CoreClasses, DISPLAYNAME_PREFIX, InputGroup, Intent, PopoverNext, popoverPropsToNextProps, refHandler, setRef, Utils, } from "@blueprintjs/core";
...
import { INVALID_DATE_MESSAGE, LOCALE, MAX_DATE, MIN_DATE, OUT_OF_RANGE_MESSAGE, OVERLAPPING_DATES_MESSAGE, } from "../dateConstants";
...
export class DateRangeInput extends DateFnsLocalizedComponent {
    ...
    render() {
        ...
        const popoverContent = (_jsx(DateRangePicker, { ...this.props, boundaryToModify: this.state.boundaryToModify, locale: locale ?? this.props.locale, onChange: this.handleDateRangePickerChange, onHoverChange: this.handleDateRangePickerHoverChange, onShortcutChange: this.handleShortcutChange, selectedShortcutIndex: selectedShortcutIndex, value: this.getSelectedRange() }));
        // allow custom props for the popover and each input group, but pass them in an order that
        // guarantees only some props are overridable.
        return (_jsx(PopoverNext, { isOpen: this.state.isOpen, placement: "bottom-start", ...popoverPropsToNextProps(popoverProps), 
            // eslint-disable-next-line jsx-a11y/no-autofocus
            autoFocus: false, className: classNames(Classes.DATE_RANGE_INPUT, popoverProps.className, this.props.className), content: popoverContent, enforceFocus: false, onClose: this.handlePopoverClose, popoverClassName: classNames(Classes.DATE_RANGE_INPUT_POPOVER, popoverProps.popoverClassName), ref: popoverRef, renderTarget: this.renderTarget }));
    }
    validateProps(props) {
        if (props.value === null) {
    ...
    renderInputGroup = (boundary) => {
        ...
        return (_jsx(InputGroup, { autoComplete: "off", disabled: inputProps?.disabled ?? this.props.disabled, fill: this.props.fill, ...inputProps, intent: this.isInputInErrorState(boundary) ? Intent.DANGER : inputProps?.intent, inputRef: this.getInputRef(boundary), onBlur: handleInputEvent, onChange: handleInputEvent, onClick: handleInputEvent, onFocus: handleInputEvent, onKeyDown: handleInputEvent, onMouseDown: handleInputEvent, placeholder: this.getInputPlaceholderString(boundary), value: this.getInputDisplayString(boundary) }));
    };
    ...
    handleInputArrowKeyDown = (e, boundary) => {
        ...
        const shiftedDate = this.getNextDateForArrowKeyNavigation(e.key, boundary) ??
    ...
    formatMinMaxDateString = (props, propName) => {
