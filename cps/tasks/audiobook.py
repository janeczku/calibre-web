    ...
    def run(self, worker_thread):
        ...
        try:
            ...
            with app.app_context():
                ...
                if not book:
                    return self._handleError(_("Book not found"))

                self.title = book.title
                self.message = _("Generating audiobook for '%(title)s'", title=self.title)

                # Get the book file data
                book_data = worker_db.get_book_format(self.book_id, self.book_format)
                if not book_data:
                    return self._handleError(_("Format %(format)s not found for book", format=self.book_format))
                ...
                if not os.path.exists(book_file):
                    return self._handleError(_("Book file not found: %(file)s", file=book_file))

                # Extract text from the book
                self.progress = 0.1
                self.message = _("Extracting text from '%(title)s'...", title=self.title)

                text = self._extract_text(book_file, self.book_format)
                if not text:
                    return self._handleError(_("Could not extract text from book"))

                # Split text into parts
                self.progress = 0.2
                self.message = _("Splitting text into parts...")
                ...
                for i, part_text in enumerate(text_parts, 1):
                    ...
                    self.message = _("Generating audio part %(current)d of %(total)d...",
                    ...
                    if audio_file:
                        ...
                        return self._handleError(_("Failed to generate audio part %(part)d", part=i))

                # Register audio files in database
                self.progress = 0.95
                self.message = _("Registering audiobook files in database...")

                for audio_file in audio_files:
                    self._register_audio_file(worker_db, book, audio_file)

                self.progress = 1.0
                self.message = _("Audiobook generated successfully: %(parts)d parts", parts=total_parts)

                return f"Generated {total_parts} audio files"

        except Exception as e:
            log.error(f"Error generating audiobook for book {self.book_id}: {str(e)}")
            return self._handleError(_("Error generating audiobook: %(error)s", error=str(e)))
