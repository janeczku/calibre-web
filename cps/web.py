...
sqlalchemy_version2 = Version(sql_version) >= Version("2")


@app.after_request
def add_security_headers(resp):
    default_src = [
        host.strip() for host in config.config_trustedhosts.split(",") if host
...
def set_bookmark(book_id, book_format):
    ...
    ub.session_commit(
        "Bookmark for user {} in book {} created".format(current_user.id, book_id)
    )
    return "", 201
...
@login_required_if_no_ano
def get_tags_json():
    return calibre_db.get_typeahead(
        db.Tags, request.args.get("q"), tag_filter=tags_filters()
    )
...
def get_languages_json():
    ...
    entries_start = [
        s for key, s in language_names.items() if s.lower().startswith(query.lower())
    ]
...
def get_matching_tags():
    ...
    q = q.filter(
        db.Books.authors.any(
            func.lower(db.Authors.name).ilike("%" + author_input + "%")
        ),
...
def query_char_list(data_colum, db_link):
    results = (
        calibre_db.session.query(
            func.upper(func.substr(data_colum, 1, 1)).label("char")
        )
        .join(db_link)
        .join(db.Books)
...
def render_hot_books(page, order):
    if current_user.check_visibility(constants.SIDEBAR_HOT):
        ...
        if current_user.show_detail_random():
            random_query = calibre_db.generate_linked_query(
...
def render_author_books(page, author_id, order):
    ...
    if services.goodreads_support and config.config_use_goodreads:
        ...
        other_books = services.goodreads_support.get_other_books(
            author_info, book_entries
        )
    return render_title_template(
        "author.html",
        entries=entries,
...
def render_series_books(page, book_id, order):
    if book_id == "-1":
        ...
        series_name = (
            calibre_db.session.query(db.Series).filter(db.Series.id == book_id).first()
        )
        if series_name:
            entries, random, pagination = calibre_db.fill_indexpage(
                page,
...
def render_category_books(page, book_id, order):
    if book_id == "-1":
        ...
        tagsname = (
            calibre_db.session.query(db.Tags).filter(db.Tags.id == book_id).first()
        )
        if tagsname:
            entries, random, pagination = calibre_db.fill_indexpage(
                page,
    ...
    if not config.config_read_column:
        ...
        try:
            if are_read:
                ...
                db_filter = (
                    coalesce(db.cc_classes[config.config_read_column].value, False)
                    != True
                )
...
def list_books():
    ...
    if sort_param == "state":
        ...
        order = (
            [db.Publishers.name.asc()]
            if order == "asc"
            else [db.Publishers.name.desc()]
        )
        join = (
            db.books_publishers_link,
            db.Books.id == db.books_publishers_link.c.book,
        ...
        order = (
            [db.Languages.lang_code.asc()]
            if order == "asc"
            else [db.Languages.lang_code.desc()]
        )
        join = (
            db.books_languages_link,
            db.Books.id == db.books_languages_link.c.book,
...
def author_list():
    if current_user.check_visibility(constants.SIDEBAR_AUTHOR):
        ...
        entries = (
            calibre_db.session.query(
                db.Authors, func.count("books_authors_link.book").label("count")
            )
...
def download_list():
    if current_user.get_view_property("download", "dir") == "desc":
        ...
        order_no = 1
    if (
        current_user.check_visibility(constants.SIDEBAR_DOWNLOAD)
        ...
        char_list = (
            ...
            .filter(
                ub.User.role.op("&")(constants.ROLE_ANONYMOUS)
...
def publisher_list():
    ...
    if current_user.check_visibility(constants.SIDEBAR_PUBLISHER):
        entries = (
            calibre_db.session.query(
                db.Publishers, func.count("books_publishers_link.book").label("count")
            )
...
def series_list():
    if current_user.check_visibility(constants.SIDEBAR_SERIES):
        ...
        if current_user.get_view_property("series", "series_view") == "list":
            entries = (
                calibre_db.session.query(
                    db.Series, func.count("books_series_link.book").label("count")
                )
            ...
            entries = sorted(
                entries, key=lambda x: x[0].name.lower(), reverse=not order_no
            )
            return render_title_template(
                "list.html",
                entries=entries,
            ...
            entries = (
                ...
                .group_by(text("books_series_link.series"))
                .having(
                    or_(func.max(db.Books.series_index), db.Books.series_index == "")
                )
                .order_by(order)
                .all()
            )
...
def language_overview():
    if (
        current_user.check_visibility(constants.SIDEBAR_LANGUAGE)
        and current_user.filter_language() == "all"
    ):
        order_no = (
            0 if current_user.get_view_property("language", "dir") == "desc" else 1
        )
        languages = calibre_db.speaking_language(
            reverse_order=not order_no, with_count=True
        )
...
def category_list():
    if current_user.check_visibility(constants.SIDEBAR_CATEGORY):
        ...
        entries = (
            calibre_db.session.query(
                db.Tags, func.count("books_tags_link.book").label("count")
            )
            ...
            try:
                ...
                try:
                    ...
                    if "surrogate" in e.reason:
                        ...
                        text_data = rawdata.decode(result["encoding"], "ignore").encode(
                            "utf-8", "ignore"
                        )
...
def send_to_ereader(book_id, book_format, convert):
    if not config.get_mail_server_configured():
        response = [
            {
                "type": "danger",
                "message": _("Please configure the SMTP mail settings first..."),
            }
        ]
            ...
            response = [
                {
                    "type": "success",
                    "message": _(
                        "Success! Book queued for sending to %(eReadermail)s",
            ...
            response = [
                {
                    "type": "danger",
                    "message": _(
                        "Oops! There was an error sending book: %(res)s", res=result
                    ),
                }
            ]
    else:
        response = [
            {
                "type": "danger",
                "message": _(
                    "Oops! Please update your profile with a valid eReader Email."
                ),
            }
        ]
    return Response(json.dumps(response), mimetype="application/json")
...
def register_post():
    ...
    try:
        ...
        return render_title_template(
            "register.html", config=config, title=_("Register"), page="register"
        )
    except (ConnectionError, Exception) as e:
        log.error("Connection error to limiter backend: %s", e)
        flash(
            _("Connection error to limiter backend, please contact your administrator"),
            category="error",
        )
        return render_title_template(
            "register.html", config=config, title=_("Register"), page="register"
    ...
    if not config.get_mail_server_configured():
        ...
        return render_title_template(
            "register.html", title=_("Register"), page="register"
        )
    nickname = (
        strip_whitespaces(to_save.get("email", ""))
        if config.config_register_email
        else to_save.get("name")
    )
    ...
    if check_valid_domain(email):
        ...
        try:
            ...
            send_registration_mail(
                strip_whitespaces(to_save.get("email", "")), nickname, password
            )
        except Exception:
            ub.session.rollback()
            flash(
        ...
        log.warning(
            'Registering failed for user "{}" Email: {}'.format(
                nickname, to_save.get("email", "")
            )
        )
        return render_title_template(
            "register.html", title=_("Register"), page="register"
...
@limiter.limit(
    "40/day",
    key_func=lambda: strip_whitespaces(request.form.get("username", "")).lower(),
)
@limiter.limit(
    "3/minute",
    key_func=lambda: strip_whitespaces(request.form.get("username", "")).lower(),
...
def login_post():
    ...
    username = (
        strip_whitespaces(form.get("username", ""))
        .lower()
        .replace("\n", "")
        .replace("\r", "")
    )
    ...
    if config.config_login_type == constants.LOGIN_LDAP and not services.ldap:
        ...
        flash(_("Cannot activate LDAP authentication"), category="error")
    user = (
        ub.session.query(ub.User).filter(func.lower(ub.User.name) == username).first()
    )
    ...
    if (
        ...
        if login_result:
            ...
            log.warning(
                'LDAP Login failed for user "%s" IP-address: %s', username, ip_address
            )
        ...
        if form.get("forgot", "") == "forgot":
            if user is not None and user.name != "Guest":
                ...
                if ret == 1:
                    flash(
                        _("New Password was sent to your email address"),
                        category="info",
                    )
                    log.info(
                        'Password reset for user "%s" IP-address: %s',
                        username,
                        ip_address,
                    )
                else:
                ...
                flash(
                    _("Please enter valid username to reset password"), category="error"
                )
                log.warning(
                    "Username missing for password reset IP-address: %s", ip_address
                )
        else:
            if (
                user
                ...
                log.warning(
                    'Login failed for user "{}" IP-address: {}'.format(
                        username, ip_address
                    )
                )
                flash(_("Wrong Username or Password"), category="error")
...
def logout():
    if current_user is not None and current_user.is_authenticated:
        ...
        if feature_support["oauth"] and (
            config.config_login_type == 2 or config.config_login_type == 3
        ):
            logout_oauth_user()
...
def change_profile(
    kobo_support, local_oauth_check, oauth_status, translations, languages
):
    to_save = request.form.to_dict()
        ...
        if current_user.role_passwd() or current_user.role_admin():
            if to_save.get("password", "") != "":
                current_user.password = generate_password_hash(
                    valid_password(to_save.get("password"))
                )
        if (
            to_save.get("kindle_mail", current_user.kindle_mail)
            != current_user.kindle_mail
        ):
...
def read_book(book_id, book_format):
    ...
    if book_format.lower() == "epub":
        ...
        return render_title_template(
            "read.html", bookid=book_id, title=book.title, bookmark=bookmark
        )
...
def show_book(book_id):
    ...
    if entries:
        ...
        book_in_shelves = []
        shelves = (
            ub.session.query(ub.BookShelf).filter(ub.BookShelf.book_id == book_id).all()
        )
        for sh in shelves:
