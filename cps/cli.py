...
def version_info():
    if _NIGHTLY_VERSION[1].startswith('$Format'):
        return "Qalibre version: %s - unknown git-clone" % _STABLE_VERSION.replace("b", " Beta")
    return "Qalibre version: %s -%s" % (_STABLE_VERSION.replace("b", " Beta"), _NIGHTLY_VERSION[1])
...
class CliParameter(object):
    ...
    def arg_parser(self):
        parser = argparse.ArgumentParser(description='Qalibre is a web app providing '
                                                     'an interface for browsing, reading and downloading eBooks\n',
                                         prog='cps.py')
        ...
        parser.add_argument('-o', metavar='path', help='path and name Qalibre logfile')
        ...
        parser.add_argument('-s', metavar='user:pass',
                            help='Sets specific username to new password and exits Qalibre')
