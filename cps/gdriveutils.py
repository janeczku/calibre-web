...
class GdriveId(Base):
    ...
    path = Column(String)
    __table_args__ = (
        UniqueConstraint('gdrive_id', 'path', name='_gdrive_path_uc'),)
    ...
    for sql in session \
        ...
        if 'CREATE TABLE gdrive_ids' in sql[0]:
            ...
            if currUniqueConstraint in sql[0]:
                sql = sql[0].replace(
                    currUniqueConstraint, 'UNIQUE (gdrive_id, path)')
                sql = sql.replace(
                    GdriveId.__tablename__, GdriveId.__tablename__ + '2')
                session.execute(sql)
                session.execute(
                    'INSERT INTO gdrive_ids2 (id, gdrive_id, path) '
...
def getEbooksFolder(drive=None):
    ...
    if drive.auth.access_token_expired:
        drive.auth.Refresh()
    ebooksFolder = "title = '%s' and 'root' in parents and " \
...
def getEbooksFolderId(drive=None):
    storedPathName = session.query(GdriveId).filter(
        GdriveId.path == '/').first()
    if storedPathName:
...
def getFolderInFolder(parentId, folderName, drive=None):
    ...
    if drive.auth.access_token_expired:
        drive.auth.Refresh()
    folder = "title = '%s' and '%s' in parents and " \
             "mimeType = 'application/vnd.google-apps.folder' " \
             "and trashed = false" % (folderName.replace("'", "\\'"), parentId)
...
def getFile(pathId, fileName, drive=None):
    ...
    if drive.auth.access_token_expired:
        drive.auth.Refresh()
    metaDataFile = "'%s' in parents and trashed = false and title = '%s'" % (
        pathId, fileName.replace("'", "\\'"))
...
def getFolderId(path, drive=None):
    ...
    storedPathName = session.query(GdriveId).filter(
        GdriveId.path == sqlCheckPath).first()
    ...
    if not storedPathName:
        ...
        for i, x in enumerate(s):
            if len(x) > 0:
                ...
                storedPathName = session.query(GdriveId).filter(
                    GdriveId.path == currentPath).first()
                if storedPathName:
                    currentFolderId = storedPathName.gdrive_id
                else:
                    currentFolderId = getFolderInFolder(
                        currentFolderId, x, drive)['id']
                    gDriveId = GdriveId()
...
def backupCalibreDbAndOptionalDownload(drive, f=None):
    ...
    if drive.auth.access_token_expired:
        drive.auth.Refresh()
    metaDataFile = "'%s' in parents and title = 'metadata.db' and " \
                   "trashed = false" % getEbooksFolderId()
...
def copyToDrive(drive, uploadFile, createRoot, replaceFiles,
    ...
    if os.path.isdir(os.path.join(prevDir, uploadFile)):
        existingFolder = drive.ListFile(
            {'q': "title = '%s' and '%s' in parents and trashed = false"
             % (os.path.basename(uploadFile), parent['id'])}) \
            .GetList()
        ...
        for f in os.listdir(os.path.join(prevDir, uploadFile)):
            if f not in ignoreFiles:
                copyToDrive(drive, f, True, replaceFiles, ignoreFiles,
                            parent, os.path.join(prevDir, uploadFile))
    else:
        if os.path.basename(uploadFile) not in ignoreFiles:
            existingFiles = drive.ListFile(
                {'q': "title = '%s' and '%s' in parents and trashed = false"
                 % (os.path.basename(uploadFile), parent['id'])}) \
                .GetList()
...
def watchFile(drive, file_id, channel_id, channel_type, channel_address,
    ...
    if expiration:
        body['expiration'] = expiration
    return drive.auth.service.files() \
        .watch(fileId=file_id, body=body) \
        .execute()
