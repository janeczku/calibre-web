...
DEFAULT_LOG_FILE  = os.path.join(constants.BASE_DIR, "calibre-web.log")
...
def create():
    parent_frame = inspect.stack(0)[1].frame
...
def setup(log_file, log_level=None):
    if log_file:
        if not os.path.dirname(log_file):
            log_file = os.path.join(constants.BASE_DIR, log_file)
    ...
    if previous_handler and getattr(previous_handler, 'baseFilename', None) == log_file:
        return
